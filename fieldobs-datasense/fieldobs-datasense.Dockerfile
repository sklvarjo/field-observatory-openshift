...
RUN --mount=type=secret,id=git_token \
    GIT_TOKEN=$(cat /run/secrets/git_token) && \
    git clone https://oauth2:${GIT_TOKEN}@github.com/ollinevalainen/fieldobs_datasense.git /fieldobs/fieldobs_datasense/
...
# FIELDOBS DATASENSE
RUN cd /fieldobs/fieldobs_datasense/ && poetry remove fieldobs-utils --lock && pip3 install .
