...
RUN --mount=type=secret,id=git_token \
    GIT_TOKEN=$(cat /run/secrets/git_token) && \
    git clone https://oauth2:${GIT_TOKEN}@github.com/ollinevalainen/fieldobs_smhi.git /fieldobs/fieldobs_smhi/
...
# FIELDOBS SMHI
RUN cd /fieldobs/fieldobs_smhi/ && poetry remove fieldobs-utils --lock && pip3 install .
