...
RUN --mount=type=secret,id=git_token \
    GIT_TOKEN=$(cat /run/secrets/git_token) && \
    git clone https://oauth2:${GIT_TOKEN}@github.com/ollinevalainen/fieldobs_radobs.git /fieldobs/fieldobs_radobs/
...
# FIELDOBS RADOBS
RUN cd /fieldobs/fieldobs_radobs/ && poetry remove fieldobs-utils --lock && pip3 install .
